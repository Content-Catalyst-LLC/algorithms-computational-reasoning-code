CREATE TABLE IF NOT EXISTS health_systems (
  system_id TEXT PRIMARY KEY,
  patient_impact REAL NOT NULL,
  population_impact REAL NOT NULL,
  clinical_validation REAL NOT NULL,
  equity_readiness REAL NOT NULL,
  privacy_readiness REAL NOT NULL,
  human_review REAL NOT NULL,
  workflow_integration REAL NOT NULL,
  monitoring REAL NOT NULL,
  governance REAL NOT NULL
);

CREATE TABLE IF NOT EXISTS health_algorithm_safety_audit (
  system_id TEXT PRIMARY KEY,
  impact_score REAL NOT NULL,
  governance_readiness_score REAL NOT NULL,
  health_algorithm_risk_score REAL NOT NULL,
  recommendation TEXT NOT NULL
);

CREATE TABLE IF NOT EXISTS health_governance_register (
  control TEXT PRIMARY KEY,
  review_question TEXT,
  status TEXT
);
