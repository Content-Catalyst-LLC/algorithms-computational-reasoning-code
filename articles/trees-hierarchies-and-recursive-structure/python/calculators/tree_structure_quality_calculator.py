...
def compute(values: list[float]) -> dict[str, float | str]:
    ...
    if quality >= 82 and risk <= 22:
        label = "strong tree-structure posture"
    elif quality >= 68 and risk <= 38:
        label = "usable tree-structure posture with review needs"
    elif risk >= 55:
        label = "high false-hierarchy risk"
    else:
        label = "partial tree-structure posture"
    return {
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Tree-structure quality and false-hierarchy-risk calculator.")
    for field in FIELDS:
    ...
    (args.output_dir / "tree_structure_quality_calculator.json").write_text(json.dumps(result, indent=2, sort_keys=True), encoding="utf-8")
