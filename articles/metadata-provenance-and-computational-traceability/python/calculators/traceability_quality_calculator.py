...
WEIGHTS = [0.12, 0.10, 0.12, 0.10, 0.08, 0.10, 0.10, 0.10, 0.10, 0.08]


def compute(values: list[float]) -> dict[str, float | str]:
    quality = max(0.0, min(100.0, 100.0 * sum(v * w for v, w in zip(values, WEIGHTS))))
    risk_fields = [values[0], values[1], values[2], values[3], values[5], values[6], values[7], values[8], values[9]]
    risk = max(0.0, min(100.0, 100.0 * mean(1.0 - v for v in risk_fields)))
    if quality >= 84 and risk <= 20:
        label = "strong traceability posture"
    elif quality >= 70 and risk <= 35:
        label = "usable traceability posture with review needs"
    elif risk >= 55:
        label = "high traceability risk"
    else:
        label = "partial traceability posture"
    return {
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Traceability quality and risk calculator.")
    for field in FIELDS:
    ...
    (args.output_dir / "traceability_quality_calculator.json").write_text(json.dumps(result, indent=2, sort_keys=True), encoding="utf-8")
